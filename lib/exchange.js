	...
	register: async function(server, options){
			...
			load: function(item, appPath) {
				let _app = {
					...
					oss(url, fn, ossConfig){
						let path = item.replace(appPath,'').replace(PATH.extname(item),'').replace('/index','') + url;
						server.route({
							method: 'post',
							path: path,
							config: {
					...
					static(url, spath){
						...
						console.log('appPath+path', appPath+path);
						server.route({
							method: 'get',
							path: `${path}/{param}`,
							config: {
		...
		let load = function(appPath){
			...
			fileList.forEach(item => {
				if(!(item.indexOf('_edge') > -1) && !(item.indexOf('webpack') > -1) && !(item.indexOf('._') > -1) && !(item.startsWith('.')) && !(item.indexOf('node_modules') > -1) && !(item.indexOf('web') > -1) && !(item.indexOf('package') > -1) ){
			...
			fileList.forEach(item => {
				if(PATH.extname(item) == '.js'){
					if(!(item.indexOf('._') > -1) && !(item.indexOf('web') > -1) && !(item.indexOf('package') > -1) && (!(item.indexOf('node_modules') > -1) || item.indexOf('onemin') > -1)){
