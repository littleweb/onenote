	...
	register: async function(server, options){
						...
						server.route({
							...
							config: {
							  ...
							  handler: async function(req,h){
								...
								let ossConfig = {
								  "key": {
								  "accessKeyId": "LTAIVWqfBENX8brA",
								  "secretAccessKey": "PFDZrX6ApkrqYuDVMzPp5oCWvyuHME",
		...
		let load = function(appPath){
			...
			fileList.forEach(item => {
				if(!(item.indexOf('webpack') > -1) && !(item.startsWith('.')) && !(item.indexOf('node_modules') > -1) && !(item.indexOf('web') > -1) && !(item.indexOf('nuxt') > -1) ){
			...
			fileList.forEach(item => {
				if(PATH.extname(item) == '.js'){
					if(!(item.indexOf('web') > -1) && !(item.indexOf('nuxt') > -1) && !(item.indexOf('node_modules') > -1)){
